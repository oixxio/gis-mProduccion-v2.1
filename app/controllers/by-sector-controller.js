		...
		$scope.changeText =  function() {
		    ...
		    if( key == 8 || key == 46 ){
		   	...
		   	if ($scope.searchText.length == 0) {
				...
				for (var i = 0; i < auxUploadData.length; i++) {
					if ( auxUploadData[i].label.match(matchText) ) {
						uploadData.push(auxUploadData[i]);
					} else {
						var aux1 = auxUploadData[i];
						for (var j = 0; j < aux1.children.length; j++) {
							if ( aux1.children[j].label.match(matchText) ) {
								uploadData.push(auxUploadData[i].children[j]);
							} else {
								var aux2 = aux1.children[j];
								for (var x = 0; x < aux2.children.length; x++) {
									if ( aux2.children[x].label.match(matchText) ) {
										uploadData.push(aux2.children[x]);
									} else {
										var aux3 = aux2.children[x];
										for (var z = 0; z < aux3.children.length; z++) {
											if ( aux3.children[z].label.match(matchText) ) {
												uploadData.push(aux3.children[z]);
												tree.expand_all();
												break;
											} else {
